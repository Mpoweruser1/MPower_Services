...
export default function MarksEntry() {
  ...
  async function createExam() {
    ...
    setSaving(false);
    if (error) { console.error('Exam creation failed:', error); setMessage(error.message || 'Failed to create exam.'); return; }
  ...
  async function saveAll() {
    ...
    const rows = students
      ...
      .map((s) => {
        ...
        // Confirmed real constraint: marks_pass_fail_check requires
        // lowercase 'pass'/'fail' — the previous capitalized
        // 'Pass'/'Fail' violated this on every single save, which is
        // why marks entry always failed. ReportCard.jsx's own
        // pass_fail comparison is fixed to match, in the same pass.
        return {
    ...
    setSaving(false);
    if (error) { console.error('Marks save failed:', error); setMessage(error.message || 'Failed to save marks. Please try again.'); return; }
