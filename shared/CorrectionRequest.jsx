...
export const STUDENT_FIELDS = [
  ...
  // village_id/village-search removed — StudentAdmission.jsx and
  // StudentDetail.jsx no longer use the uuid+FK village_id column at
  // all (that table is CTS's electoral village list, scoped to
  // registered constituencies, and was never actually populated at
  // admission). village_name/mandal_name/district_name/state are the
  // real plain-text columns now in use — same simple free-text
  // pattern as father_name/parent_phone below, since none of these
  // are foreign keys.
  { key: 'village_name',   label: 'Village / Town' },
...
export default function CorrectionRequest({
  ...
  // villageQuery/villageResults/villageDisplay/searchVillage/
  // selectVillage removed — no field now uses 'village-search' (see
  // STUDENT_FIELDS above). The village/mandal/district/state fields
  // now in use are plain text and go through the ordinary text-input
  // branch below, same as father_name/parent_phone.
