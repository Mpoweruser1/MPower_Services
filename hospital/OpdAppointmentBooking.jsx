...
export default function OpdAppointmentBooking() {
  ...
  async function load() {
    ...
    if (data) {
      const { data: slotRows } = await supabase
        .from('opd_appointment_slots_public').select('id, slot_time, status')
  ...
  async function book() {
    ...
    const { error: bookErr, count } = await supabase
      ...
      .update({ status: 'booked', booked_name: name.trim(), booked_phone: phone.trim(), reason: reason.trim() || null, booked_at: new Date().toISOString() }, { count: 'exact' })
    ...
    // Confirmation built from data already on hand — selectedSlot was
    // already loaded before booking, so there's no need to read the
    // row back from the server at all, avoiding any dependency on
    // anonymous SELECT access to the real table.
